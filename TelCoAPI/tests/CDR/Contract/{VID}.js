...
/**
 * Test for /CDR/Contract/{VID}
 */
Test('/CDR/Contract/{VID}', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         * responses: 200
         */
        t.test('test GetContract get operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/CDR/Contract/{VID}']['get']['responses']['200']['schema']);
         ...
         * responses: 200
         */
        t.test('test CreateContact post operation', function (t) {
            Mockgen().requests({
                path: '/CDR/Contract/{VID}',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/CDR/Contract/{VID}']['post']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
